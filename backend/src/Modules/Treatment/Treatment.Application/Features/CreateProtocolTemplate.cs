...
/// <summary>
/// Command to create a new treatment protocol template.
/// </summary>
public sealed record CreateProtocolTemplateCommand(
    string Name,
    int TreatmentType,
...
/// <summary>
/// Validator for <see cref="CreateProtocolTemplateCommand"/>.
/// Enforces business rules: session count 1-6, deduction percent 10-20, prices non-negative.
/// </summary>
public class CreateProtocolTemplateCommandValidator : AbstractValidator<CreateProtocolTemplateCommand>
{
    public CreateProtocolTemplateCommandValidator()
    {
        RuleFor(x => x.Name)
...
/// <summary>
/// Wolverine static handler for creating a new treatment protocol template.
/// Validates input, creates entity via factory, persists, maps to DTO.
/// </summary>
public static class CreateProtocolTemplateHandler
{
    public static async Task<Result<TreatmentProtocolDto>> Handle(
        CreateProtocolTemplateCommand command,
        ITreatmentProtocolRepository repository,
        IUnitOfWork unitOfWork,
        IValidator<CreateProtocolTemplateCommand> validator,
        ...
        CancellationToken ct)
    {
        ...
        var protocol = TreatmentProtocol.Create(
            command.Name,
    ...
    internal static TreatmentProtocolDto MapToDto(TreatmentProtocol protocol) =>
        new(
            Id: protocol.Id,
            Name: protocol.Name,
            TreatmentType: protocol.TreatmentType.ToString(),
            DefaultSessionCount: protocol.DefaultSessionCount,
            PricingMode: protocol.PricingMode.ToString(),
            DefaultPackagePrice: protocol.DefaultPackagePrice,
            DefaultSessionPrice: protocol.DefaultSessionPrice,
            MinIntervalDays: protocol.MinIntervalDays,
            MaxIntervalDays: protocol.MaxIntervalDays,
            DefaultParametersJson: protocol.DefaultParametersJson ?? string.Empty,
            CancellationDeductionPercent: protocol.CancellationDeductionPercent,
