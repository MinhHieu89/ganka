...
import { useConsumableAlerts } from "@/features/consumables/api/consumables-queries"

export function ConsumableAlertBanner() {
  const [open, setOpen] = useState(false)
  const { data: alerts, isLoading } = useConsumableAlerts()
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 flex-1 min-w-0">
            ...
            {count > 0 ? (
              ...
              <span className="text-xs text-muted-foreground">Tồn kho ổn định</span>
            )}
          </div>
